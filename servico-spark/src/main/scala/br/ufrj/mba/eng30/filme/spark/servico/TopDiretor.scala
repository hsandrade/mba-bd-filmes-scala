 ...
 */
object TopDiretor extends NewSparkJob {
  ...
  def runJob(sc: SparkContext, runtime: JobEnvironment, data: JobData): JobOutput = {
    ...
    fileDf.createOrReplaceTempView("topDiretor")
    
    //executa consulta 
    val queryDf = spark.sql("select distinct director_name, sum(gross) as gross_total, count(movie_title) as qtd_movie " 
        + "from topDiretor " 
        + "group by director_name " 
        + "order by gross_total desc limit " + data)
